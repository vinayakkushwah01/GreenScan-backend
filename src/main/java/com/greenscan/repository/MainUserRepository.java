...
public interface MainUserRepository extends JpaRepository<MainUser, Long> {
    ...
    @Query("SELECT u FROM MainUser  u WHERE u.role = :role AND u.city IN :cities AND u.isActive = true")
    List<MainUser> findActiveUsersByRoleAndCities(@Param("role") UserRole role, @Param("cities") List<String> cities);
    
    @Query("SELECT u FROM MainUser  u WHERE u.role = 'VENDOR' AND u.city = :city AND u.isActive = true")
    List<MainUser> findActiveVendorsByCity(@Param("city") String city);
    
    @Modifying
    @Query("UPDATE MainUser  u SET u.lastLogin = :loginTime WHERE u.id = :userId")
    void updateLastLogin(@Param("userId") Long userId, @Param("loginTime") LocalDateTime loginTime);
    
    @Modifying
    @Query("UPDATE MainUser  u SET u.failedLoginAttempts = :attempts WHERE u.id = :userId")
    void updateFailedLoginAttempts(@Param("userId") Long userId, @Param("attempts") Integer attempts);
    
    @Query("SELECT COUNT(u) FROM MainUser  u WHERE u.role = :role AND u.isActive = true")
