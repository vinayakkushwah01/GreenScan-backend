...
public class OtpService {
   ...
   public boolean validateOtp(MainUser user, String otpCode) {
    ...
    //Verify email otp
    @Transactional
    void generateAndMailOtp(MainUser user){
     Random random = new Random();
        int otpInt = 100000 + random.nextInt(900000);
        String otpCode = String.valueOf(otpInt);
        ...
        + "    <p>Use the <strong>One-Time Password (OTP)</strong> below to verify your Email. For security, this code will expire in <strong>5 minutes</strong>.</p>"
        + "    <div class='otp-box'>" + otpCode + "</div>"
        + "    <p>If you did not request a Email verification, please safely ignore this email.</p>"
        + "    <p class=\"platform-info\">"
        + "        Scan and recycle for the future! "
        + "        <br>A recycle product sell platform where you can gain rewards."
        + "    </p>"
        ...
        try {
            ...
            throw new MailSendException("Failed to prepare or send  email verification .");
        }
        catch (Exception e) {
        // Catch other potential errors (like IO errors during setup)
         throw new MailSendException("An unexpected error occurred during email creation/sending at email verification.");
        }
    }
}